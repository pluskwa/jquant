...
public class BottomParameter extends CalculatedParameter {
    ...
    public BottomParameter(String symbol) {
        this.symbol = symbol;
    ...
    public Key getQueryKey() {
        return Key.from("Bottom", symbol, "day");
    }
    ...
    public double calculate(Date date) {
        return Math.min(DataService.instance().value(date, open), DataService.instance().value(date, close));
    }
}
